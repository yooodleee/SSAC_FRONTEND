'use client';

import { useEffect, useRef, useState } from 'react';
import { useRouter } from 'next/navigation';
import type { components } from '@/../api-contract/generated/api-types';
import { onboardingService } from '@/services/onboarding';
import { LandingHeader } from '@/components/layout/LandingHeader';
import { cn } from '@/lib/utils';

type QuestionDto = components['schemas']['QuestionDto'];
type UserType = 'HIGH_SCHOOL' | 'EARLY_CAREER';
type TestStep = 'question' | 'complete';

// UI-only type: not derived from API contract
interface FlowAnswer {
  questionIndex: number;
  optionIndex: number;
}

// UI-only type: not derived from API contract
interface TestState {
  step: TestStep;
  currentQuestion: number; // 1-indexed
  answers: FlowAnswer[];
}

const SESSION_TEST_KEY = 'onboarding_test_state';
const SESSION_USER_TYPE_KEY = 'onboarding_user_type';
const SESSION_SUBMIT_KEY = 'onboarding_answers';

function readPersistedTest(): TestState | null {
  if (typeof window === 'undefined') return null;
  try {
    const raw = sessionStorage.getItem(SESSION_TEST_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as TestState;
  } catch {
    return null;
  }
}

function getInitialTestState(): TestState {
  const persisted = readPersistedTest();
  if (!persisted || persisted.step === 'complete') {
    return { step: 'question', currentQuestion: 1, answers: [] };
  }
  return persisted;
}

function readUserType(): UserType | null {
  if (typeof window === 'undefined') return null;
  const val = sessionStorage.getItem(SESSION_USER_TYPE_KEY);
  if (val === 'HIGH_SCHOOL' || val === 'EARLY_CAREER') return val;
  return null;
}

// ─── SVG Chevrons ────────────────────────────────────────────────────────────

function ChevronLeft() {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M15 18l-6-6 6-6" />
    </svg>
  );
}

function ChevronRight() {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M9 18l6-6-6-6" />
    </svg>
  );
}

// ─── Option Button ────────────────────────────────────────────────────────────

interface OptionButtonProps {
  label: string;
  selected: boolean;
  onClick: () => void;
}

function OptionButton({ label, selected, onClick }: OptionButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        'w-full min-h-[48px] rounded-lg border px-4 py-3 text-sm font-medium text-white text-left',
        'transition-colors [transition-duration:200ms]',
        'motion-reduce:transition-none',
        selected
          ? 'border-[#1B4332] bg-[#1B4332]'
          : 'border-white/20 bg-white/10 hover:bg-white/15',
      )}
      aria-pressed={selected}
    >
      {label}
    </button>
  );
}

// ─── Question Step ───────────────────────────────────────────────────────────

interface QuestionStepProps {
  question: QuestionDto | undefined;
  currentQuestion: number;
  totalQuestions: number;
  selectedOptionIndex: number | null;
  onSelect: (optionIndex: number) => void;
  isLoading: boolean;
}

const OPTION_LABELS = ['①', '②', '③', '④', '⑤'];

function QuestionStep({
  question,
  currentQuestion,
  totalQuestions,
  selectedOptionIndex,
  onSelect,
  isLoading,
}: QuestionStepProps) {
  if (isLoading) {
    return (
      <div className="flex w-full max-w-[650px] flex-col gap-4 self-center">
        <div className="h-1.5 w-full animate-pulse rounded-full bg-white/20" />
        <div className="h-5 w-3/4 animate-pulse rounded bg-white/20" />
        {[0, 1, 2].map((i) => (
          <div key={i} className="h-[48px] animate-pulse rounded-lg bg-white/10" />
        ))}
      </div>
    );
  }

  if (!question) return null;

  return (
    <div className="flex flex-col items-center gap-6">
      {/* Progress bar — full width */}
      <div
        className="h-1.5 w-full max-w-[650px] overflow-hidden rounded-full bg-white/20"
        role="progressbar"
        aria-valuenow={currentQuestion}
        aria-valuemin={1}
        aria-valuemax={totalQuestions}
        aria-label={`${totalQuestions}문제 중 ${currentQuestion}번째`}
      >
        <div
          className="h-full rounded-full bg-[#4CAF82] transition-all duration-300 motion-reduce:transition-none"
          style={{ width: `${(currentQuestion / totalQuestions) * 100}%` }}
        />
      </div>

      {/* Question */}
      <p className="w-full max-w-[650px] text-base font-medium leading-relaxed text-white">
        Q. {question.content}
      </p>

      {/* Options */}
      <div className="flex w-full max-w-[650px] flex-col gap-3">
        {(question.options ?? []).map((opt, idx) => (
          <OptionButton
            key={opt.id ?? idx}
            label={`${OPTION_LABELS[idx] ?? `(${idx + 1})`} ${opt.label ?? ''}`}
            selected={selectedOptionIndex === idx}
            onClick={() => onSelect(idx)}
          />
        ))}
      </div>
    </div>
  );
}

// ─── Complete Step (Login Prompt) ─────────────────────────────────────────────

function CompleteStep({ onLogin }: { onLogin: () => void }) {
  return (
    <div className="flex flex-1 flex-col">
      <div className="flex flex-1 items-center justify-center">
        <p className="text-center text-white" style={{ fontSize: '22px', fontWeight: 700 }}>
          지금 로그인하고
          <br />
          점수를 확인해보세요
        </p>
      </div>
      <button
        type="button"
        onClick={onLogin}
        className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-40px)] max-w-[390px] rounded-lg font-semibold text-white"
        style={{ backgroundColor: '#1B4332', height: '48px' }}
      >
        로그인
      </button>
    </div>
  );
}

// ─── Main OnboardingTestFlow ──────────────────────────────────────────────────

export function OnboardingTestFlow({ isLoggedIn }: { isLoggedIn: boolean }) {
  const router = useRouter();

  // Lazy-initialize from sessionStorage to avoid setState-in-effect
  const [testState, setTestState] = useState<TestState>(getInitialTestState);
  const [questions, setQuestions] = useState<QuestionDto[] | null>(null);
  const [isLoadingQuestions, setIsLoadingQuestions] = useState(true);
  const [toastMessage, setToastMessage] = useState<string | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const { step, currentQuestion, answers } = testState;

  // Persist test state to sessionStorage on every change
  const isFirstRender = useRef(true);
  useEffect(() => {
    if (isFirstRender.current) {
      isFirstRender.current = false;
      return;
    }
    sessionStorage.setItem(SESSION_TEST_KEY, JSON.stringify(testState));
  }, [testState]);

  // Fetch questions on mount
  const didFetchRef = useRef(false);
  useEffect(() => {
    if (didFetchRef.current) return;
    didFetchRef.current = true;

    const userType = readUserType();
    if (!userType) {
      // No userType in session — go back to type selection
      router.replace('/onboarding');
      return;
    }

    async function fetchQuestions() {
      try {
        const data = await onboardingService.getQuestions(userType!);
        setQuestions(data.questions ?? []);
      } catch (err) {
        const error = err as { code?: string; status?: number };
        if (error.code === 'ONBOARDING-001') {
          router.replace('/signup/type');
          return;
        }
        if (error.code === 'ONBOARDING-002') {
          // 이미 온보딩 완료 — 결과 페이지로 이동
          router.replace('/onboarding/result');
          return;
        }
        if (error.status === 401) {
          router.replace('/onboarding');
          return;
        }
        setToastMessage('문제를 불러오지 못했어요. 잠시 후 다시 시도해주세요.');
      } finally {
        setIsLoadingQuestions(false);
      }
    }

    void fetchQuestions();
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // Auto-dismiss toast
  useEffect(() => {
    if (!toastMessage) return;
    const timer = setTimeout(() => setToastMessage(null), 2000);
    return () => clearTimeout(timer);
  }, [toastMessage]);

  const totalQuestions = questions?.length ?? 5;
  const currentIdx = currentQuestion - 1;
  const currentQ = questions?.[currentIdx];
  const isLastQuestion = currentQuestion === totalQuestions;
  const currentAnswer = answers.find((a) => a.questionIndex === currentIdx);

  function handleSelectOption(optionIndex: number) {
    setTestState((prev) => ({
      ...prev,
      answers: [
        ...prev.answers.filter((a) => a.questionIndex !== currentIdx),
        { questionIndex: currentIdx, optionIndex },
      ],
    }));
  }

  function handleComplete() {
    if (!currentAnswer) {
      setToastMessage('답변을 선택해주세요');
      return;
    }

    const allAnswers = [
      ...answers.filter((a) => a.questionIndex !== currentIdx),
      { questionIndex: currentIdx, optionIndex: currentAnswer.optionIndex },
    ];

    if (!isLoggedIn) {
      // Save for post-login submission
      const userType = readUserType();
      sessionStorage.setItem(
        SESSION_SUBMIT_KEY,
        JSON.stringify({
          userType,
          answers: allAnswers.map((a) => ({
            questionIndex: a.questionIndex,
            optionIndex: a.optionIndex,
          })),
        }),
      );
      setTestState((prev) => ({ ...prev, step: 'complete', answers: allAnswers }));
      return;
    }

    // Logged in — map to real IDs and submit immediately
    if (!questions?.length) {
      setToastMessage('문제를 불러오는 중입니다. 잠시 후 다시 시도해주세요.');
      return;
    }

    const mappedAnswers = allAnswers
      .map((a) => {
        const q = questions[a.questionIndex];
        if (!q || q.id === undefined) return null;
        const opt = (q.options ?? [])[a.optionIndex];
        if (!opt?.id) return null;
        return { questionId: q.id, selectedOption: opt.id };
      })
      .filter((a): a is { questionId: number; selectedOption: string } => a !== null);

    if (!mappedAnswers.length) {
      setToastMessage('답변 매핑에 실패했어요. 다시 시도해주세요.');
      return;
    }

    setIsSubmitting(true);
    onboardingService
      .submitAnswers({ answers: mappedAnswers })
      .then(() => {
        sessionStorage.removeItem(SESSION_TEST_KEY);
        sessionStorage.removeItem(SESSION_USER_TYPE_KEY);
        router.replace('/onboarding/result');
      })
      .catch((err: unknown) => {
        const error = err as { code?: string };
        if (error.code === 'ONBOARDING-002') {
          router.replace('/onboarding/result');
          return;
        }
        setToastMessage('제출에 실패했어요. 잠시 후 다시 시도해주세요.');
      })
      .finally(() => setIsSubmitting(false));
  }

  function handleNext() {
    if (step === 'question') {
      if (!currentAnswer) {
        setToastMessage('답변을 선택해주세요');
        return;
      }
      if (!isLastQuestion) {
        setTestState((prev) => ({ ...prev, currentQuestion: prev.currentQuestion + 1 }));
        return;
      }
      handleComplete();
    }
  }

  function handlePrev() {
    if (step === 'question') {
      if (currentQuestion === 1) {
        router.push('/onboarding');
        return;
      }
      setTestState((prev) => ({ ...prev, currentQuestion: prev.currentQuestion - 1 }));
      return;
    }
    if (step === 'complete') {
      setTestState((prev) => ({ ...prev, step: 'question', currentQuestion: totalQuestions }));
    }
  }

  // ─── Render ───────────────────────────────────────────────────────────────

  return (
    <>
      <LandingHeader isLoggedIn={isLoggedIn} />

      <div
        className="flex flex-col"
        style={{ minHeight: '100vh', paddingTop: '64px', backgroundColor: '#1A1A1A' }}
      >
        {/* Image section — 25vh with dark overlay */}
        <div className="relative flex-shrink-0" style={{ height: '25vh' }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/images/onboarding.png"
            alt=""
            aria-hidden="true"
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
          />
          <div
            className="absolute inset-0"
            aria-hidden="true"
            style={{ background: 'rgba(0, 0, 0, 0.4)' }}
          />

          {/* Chevron navigation bar */}
          <div className="absolute left-0 right-0 top-0 flex items-center justify-between px-4 py-3 text-white">
            <button
              type="button"
              onClick={handlePrev}
              aria-label="이전"
              className="rounded p-1 transition-colors duration-200 hover:bg-white/10 active:bg-white/20 motion-reduce:transition-none"
            >
              <ChevronLeft />
            </button>

            <div className="flex items-center gap-2">
              {step === 'question' && (
                <span className="text-sm font-medium" aria-hidden="true">
                  {currentQuestion} / {totalQuestions}
                </span>
              )}

              {step === 'question' && isLastQuestion ? (
                <button
                  type="button"
                  onClick={handleComplete}
                  disabled={isSubmitting}
                  className="font-semibold text-white transition-[filter] duration-200 hover:brightness-110 active:brightness-90 motion-reduce:transition-none disabled:opacity-60"
                  style={{
                    backgroundColor: '#1B4332',
                    borderRadius: '6px',
                    padding: '8px 16px',
                    fontSize: '14px',
                  }}
                >
                  완료
                </button>
              ) : step === 'question' ? (
                <button
                  type="button"
                  onClick={handleNext}
                  aria-label="다음"
                  className="rounded p-1 transition-colors duration-200 hover:bg-white/10 active:bg-white/20 motion-reduce:transition-none"
                >
                  <ChevronRight />
                </button>
              ) : null}
            </div>
          </div>
        </div>

        {/* Dark content section */}
        <div
          className="flex flex-1 flex-col px-5 py-8"
          style={{ backgroundColor: '#1A1A1A', color: '#FFFFFF' }}
        >
          {step === 'question' && (
            <QuestionStep
              question={currentQ}
              currentQuestion={currentQuestion}
              totalQuestions={totalQuestions}
              selectedOptionIndex={currentAnswer?.optionIndex ?? null}
              onSelect={handleSelectOption}
              isLoading={isLoadingQuestions}
            />
          )}
          {step === 'complete' && (
            <CompleteStep onLogin={() => router.push('/login?redirectTo=/onboarding/submit')} />
          )}
        </div>

        {/* Toast */}
        {toastMessage && (
          <div
            role="alert"
            aria-live="assertive"
            className="fixed bottom-24 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-black/80 px-4 py-2 text-sm text-white backdrop-blur-sm"
          >
            {toastMessage}
          </div>
        )}
      </div>
    </>
  );
}
